...
from .models import jiaotongzhaoshi
import codecs

def index(request):
    return render(request,'jiaotongzhaoshi.html')
...
def results(request):
    ...
    objects=jiaotongzhaoshi.objects.all()
    if sheng!='省':
    ...
    return render(request,'results_jtzs.html',{'x':objects,'n':n,'wushi':50})

def search(request):
    reaing=request.GET.get('reaing',0)
    return HttpResponse(str(reaing))

def result_jtzs(request):
    id = request.GET.get('id', 0)
    object=jiaotongzhaoshi.objects
    anli=object.filter(id=id)[0]
    dir='F:\wenshu\jiaotongzhaoshi-all/'+anli.filename.split('.')[0]+'.txt'
    if anli.Dentention_huanxing == '0.0':
        anli.Dentention_huanxing = 'unknown'
    if anli.FixtedTerm_huanxing == '0.0':
        anli.FixtedTerm_huanxing = 'unknown'
